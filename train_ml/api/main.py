...
def create_app() -> FastAPI:
    tags_meta = [
        {
            "name": "Train ML API",
            "description": "TrainML entrypoint API"
        }
    ]
    ...
    origins = ["http://localhost:3002", "http://localhost:29083", "http://localhost:5173", os.getenv("FRONTEND_ENDPOINT")]
    app.add_middleware(
        CORSMiddleware,
...
if __name__ == "__main__":
    import os
    uvicorn.run("main:app", host="0.0.0.0", port=int(os.getenv('API_PORT')), log_level="debug", reload=True)